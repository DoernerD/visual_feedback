...
class P_Controller(object):
    ...
    def refPoseCb(self, pose):
        # Pose in NED (compared to ENU of the state feedback)
        self.refX = pose.pose.position.x
        self.refY = pose.pose.position.y
    ...
    def publishControlAction(self, u):
        ...
        thruster1.rpm = u[0]
        thruster2.rpm = u[0]
        vec.thruster_horizontal_radians = self.vecHorizontalNeutral
        vec.thruster_vertical_radians = u[1]
        vbs.value = u[2]
        lcg.value = u[3]
    ...
    def computeControlAction(self):
        # u = [thruster, vec, vbs, lcg]
        # x = [x, y, z, roll, pitch, yaw]

        Kp = np.array([35,50,100,10])        # P control gain
        Ki = np.array([0.5,1,0.5,1])                # I control gain
        u = np.array([0., 0., 0., 0.])
        ...
        u[0] = (Kp[0]*self.err[0] + Ki[0]*self.integral[0] \
            + Kp[0]*self.err[1] + Ki[0]*self.integral[1])   # PI control thrusters
        u[1] = (Kp[1]*self.err[5] + Kp[1]*self.err[5] \
            + Ki[1]*self.integral[5] + Ki[1]*self.integral[5])  # PI control vectoring
        u[2] = Kp[2]*self.err[2] + Ki[2]*self.integral[2]   # PI control vbs
        u[3] = Kp[3]*self.err[4] + Ki[3]*self.integral[4]   # PI control lcg
    ...
    def limitControlAction(self,u):
        ...
        print("Control Input:", np.around(u, decimals=3))
